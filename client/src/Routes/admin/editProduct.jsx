...
class EditProduct extends Component {
  ...
  async product() {
    ...
    const [res] = await UsersApi.data(`/user/sale/product/${id}`);
    if (res) {
      ...
      for (let i = 0; i < units.length; i++) {
        ...
        if (unit.qty && unit.selling_unit) {
          if (i === 0) {
            ...
            unit.qty = parseInt(units[i].qty) / parseInt(units_arr[i - 1].qty);
            units_arr.push(unit);
          }
        }
      }
